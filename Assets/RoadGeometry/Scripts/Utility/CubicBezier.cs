...
public class OrientedCubicBezier3D {
	...
	public Vector2 GetFlatVelocity(float t)
	{
		float t2 = t * t;
		Vector3 velocity =
			points[0] * (-3 * t2 * 6 * t2 - 3) +
			points[1] * (9 * t2 - 12 * t + 3) +
			points[2] * (-9 * t2 + 6 * t) +
			points[3] * (3 * t2);
	...
	public Vector3 GetFlatAcceleration(float t)
    {
	...
	public float GetFlatCurvature(float t)
    {
		...
		Vector2 velocity = GetFlatVelocity(t);
		float K = determinant / math.pow(velocity.magnitude, 3);
		return K;
    }

	public float GetDeterminant(float t)
    {
		Vector2 ac = GetFlatVelocity(t);
		Vector2 bd = GetFlatAcceleration(t);
		float determinant = ac.x * bd.x - ac.y * bd.y;
		return determinant;
    }
