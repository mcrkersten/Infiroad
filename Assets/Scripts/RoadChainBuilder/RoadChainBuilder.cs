...
public class RoadChainBuilder : MonoBehaviour
{
    ...
    private void InstantiateAssetPools()
    {
        ...
        objectPooler.InstantiateAssetSpawnPointPool(road.assetSpawnPointPoolSize, road.assetSpawnPoint);
        ...
        if (road.segmentVariation)
        {
            foreach (VariationSettings s in road.roadVariation)
                foreach (VariationSettings.Variation v in s.roadSettings)
                    objectPooler.InstantiatePool(v.roadSettings.assetPools, v.roadSettings.roadTypeTag);
        }
        else
        {
            objectPooler.InstantiatePool(road.mainRoadSettings.assetPools, road.mainRoadSettings.roadTypeTag);
        }
    }
