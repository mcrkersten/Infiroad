...
public class RoadChain : MonoBehaviour {
	...
	public void InitializeAllSegments(RoadSettings settings, List<RoadSegment> roadSegments)
    {
		...
		foreach (RoadSegment seg in organizedSegments)
        {
			...
			SpawnAssetPointColliders(settings, seg);
		}
	}
	...
	public void InitializeSegment(RoadSettings settings, RoadSegment roadSegment)
    {
		...
		SpawnAssetPointColliders(settings, roadSegment);
	}
	...
	public void SpawnDecor(RoadSegment segment, RoadDecoration decoration, int segmentIndex)
    {
		...
		RoadChainBuilder rcb = RoadChainBuilder.instance;
		int totalLoops = rcb.generatedRoadEdgeloops;

		foreach (Decoration item in decoration.decor)
		{
			int edgeLoop = segmendLoops - (int)Mathf.Lerp(0, segmendLoops, 1f - Mathf.Clamp01(decoration.mainCurveTime + item.curveTime));
			NoiseGenerator ng = roadSettings.generatorInstance;
			Vector3 noise = Vector3.zero;

			foreach (NoiseChannelSettings nts in roadSettings.noiseChannels[item.noiseChannel].channelSettings)
			{
				noise += ng.getNoise(segment.startEndLoop.x + edgeLoop, nts);
			}

			OrientedPoint orientedPoint = GetOrientedPointOnRoad(Mathf.Clamp01(decoration.mainCurveTime + item.curveTime), segmentIndex, segment.roadSetting.rotationEasing);
			Vector3 localPoint = new Vector3(item.position.x, item.position.y, 0);
			...
			GameObject inst = Instantiate(item.prefab, worldPosition, segment.transform.rotation * orientedPoint.rot, segment.transform);
			if (item.prefab.gameObject.CompareTag("StartPosition"))
				rcb.vehicleStartTransform = inst.transform;
		}
	}


	#region Initialize
	public void SpawnAssetPointColliders(RoadSettings roadSettings, RoadSegment segment)
    {
        ...
        foreach (AssetSpawnPoint item in spawnEdge.assetSpawnPoints)
        {
            if (item.spawnBetweenPoints)
            {
				for (float i = 0.5f; i <= item.spawnPointsBetweenAmount; i++)
				{
					...
					AssetSpawnPointTag x = ObjectPooler.Instance.SpawnFromAssetSpawnPointPool(point, item.assetPointType);
					x.gameObject.name = road.roadTypeTag;
				}
            }
            else
            {
				AssetSpawnPointTag x = ObjectPooler.Instance.SpawnFromAssetSpawnPointPool(spawnEdge.leftPoint, item.assetPointType);
				x.gameObject.name = road.roadTypeTag;
			}
		}
	}
